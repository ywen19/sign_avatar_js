...
# Defaults for this repository / comparison.blend.
# This absolute path keeps the script usable from Blender's Text Editor,
# where __file__ may not point at this repository.
DEFAULT_JSON_PATH = Path("/home/ywen/Desktop/sign_avatar_js/vocabs/art_gallery_0.json")
DEFAULT_ARMATURE_OBJECT_NAME = "SMPLX-lh-female"

# The vocab JSON stores quaternions as [x, y, z, w].
JSON_QUATERNION_ORDER = "xyzw"

# Change these if you want to keep old actions or offset the animation.
CLEAR_EXISTING_ANIMATION = True
CLEAR_POSE_BEFORE_APPLY = True
FRAME_OFFSET = 0
SET_INTERPOLATION_LINEAR = True
...
def set_action_interpolation_linear(armature):
    ...
    if fcurves is None:
        print("[WARN] Could not set interpolation: this Blender Action has no direct fcurves API")
        return
...
def apply_motion(data, armature):
    ...
    if CLEAR_EXISTING_ANIMATION or CLEAR_POSE_BEFORE_APPLY:
    ...
    if SET_INTERPOLATION_LINEAR:
        set_action_interpolation_linear(armature)

    bpy.context.view_layer.update()

    print("SMPL-X vocab motion applied")
...
def main():
    ...
    print(f"Loading motion JSON: {json_path}")
