...
def collapse_repeated_underscores(name):
...
def plan_operations(root_path):
    ...
    for old_name in folder_names:
        collapsed_name = collapse_repeated_underscores(old_name)

        if collapsed_name == old_name:
            continue
        ...
        final_target = build_unique_name(collapsed_name, used_names, duplicate_groups)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collapse repeated underscores in folder names and matching files."
    )
    parser.add_argument(
        "root_path",
