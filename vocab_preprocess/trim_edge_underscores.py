...
def trim_edge_underscores(name):
    return name.strip("_")
...
def split_file_suffix(file_name, old_folder_name):
    ...
    if base_name.startswith(prefix):
        suffix = base_name[len(old_folder_name):]  # keep leading "_"
        return suffix, ext
...
def plan_operations(root_path):
    ...
    for old_name in folder_names:
        trimmed_name = trim_edge_underscores(old_name)

        if trimmed_name == old_name:
            continue

        if not trimmed_name:
            # Skip names that would become empty after trimming.
            continue

        used_names.remove(old_name)
        final_target = build_unique_name(trimmed_name, used_names, duplicate_groups)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Trim leading/trailing underscores from folder names and matching files."
    )
    parser.add_argument(
        "root_path",
