...
def plan_operations(root_path):
    ...
    for old_name in folder_names:
        lower_name = to_lowercase(old_name)

        if lower_name == old_name:
            continue

        used_names.remove(old_name)
        final_target = build_unique_name(lower_name, used_names, duplicate_groups)
...
def merge_duplicate_groups(existing_groups, new_groups):
    ...
    for key, values in existing_groups.items():
        ...
        for value in values:
            ...
            if lower_value not in merged[lower_key]:
                merged[lower_key].append(lower_value)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Lowercase folder names and matching files."
    )
    parser.add_argument(
        "root_path",
